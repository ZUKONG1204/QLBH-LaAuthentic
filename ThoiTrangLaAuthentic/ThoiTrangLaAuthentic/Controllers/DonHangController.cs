...
namespace ThoiTrangLaAuthentic.Controllers
{
    public class DonHangController : Controller
    {
        ...
        public DonHangController(ThoitrangLaAuthenticDbContext context, INotyfService notyfService)
        {
            _context = context;
        ...
        public async Task<IActionResult> Details (int? id)
        {
            ...
            try
            {
                ...
                var chitietdonhang = _context.OrderDetails
                    .Include(x => x.Products).AsNoTracking().Where(x => x.OrderId == id).OrderBy(x => x.OrderDetailId).ToList();
